...
class Sensor
{
    ...
    public function mrtg_output($params, $withconfig = false)
    {
    ...
    public function cpuusage($aspercent = false)
    {
    ...
    public function memusage($aspercent = false)
    {
        // >free
    ...
    public function diskusage($path = false, $aspercent = false)
    {
        // >df
    ...
    public function foldersize($folder, $options)
    {
    ...
    public function filecount($folder, $options)
    {
        if (! file_exists($folder)) {
            return false;
        }
    ...
    public function proccount($filter = false)
    {
        $server = strtolower($this->params['server']);
        $ss = new OStools;
    ...
    public function pingtime($address, $port = 80)
    {
        if (! $address) {
            return false;
        }
    ...
    public function foldercount($folder, $options)
    {
        if (! file_exists($folder)) {
            return false;
        }
    ...
    public function digest($text, $length = 4, $keep = 1)
    {
        if ($keep == 0) {
    ...
    public function parse_cmd($cmd, $folder = false, $cachesecs = 300, $linenr = 1, $fieldnr = 1)
    {
    ...
    public function tcpping($ip, $port)
    {
        $timeout = 4;
        $t1 = microtime(true);
